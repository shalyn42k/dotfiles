...
ShellRoot {
    ...
    function select(name) {
        ...
        // "rm -f .../rigswitch-*.log" — старые логи от прошлых запусков
        // оверлея (каждый select() берёт новый уникальный путь, старые
        // никто не подчищал). Паттерн жёстко привязан к нашему префиксу и
        // к конкретному runtimeDir — глоб не может задеть ничего чужого.
        // Идёт ДО "> лог" ниже, так что свежесозданный файл под удаление
        // попасть не может — его ещё не существует в момент rm.
        // ": > лог" следующим шагом — гарантирует, что файл создан/пуст
        // раньше, чем свитч в него пишет (и раньше, чем наш tail -F вообще
        // успеет запуститься).
        Quickshell.execDetached(["sh", "-c",
            "rm -f '" + runtimeDir + "'/rigswitch-*.log; " +
        ...
        //
        // timeout 120 — самоисцеляющийся потолок жизни ИМЕННО tail, не
        // свитча. Если нас убьют извне (повторный бинд = pkill по cmdline
        // `qs -c rigswitch` — под него tail не попадает, finishSwitch() тоже
        // не успевает выполниться), tail осиротеет и, поскольку -F никогда
        // не завершается по EOF, будет висеть вечно + держать inotify-вотч.
        // 120с — с большим запасом покрывает самую медленную стадию (daemons:
        // session.sh stop старого рига + start нового) и всё же остаётся
        // жёстким потолком, а не ещё одним таймером прогресса.
        tailProc.command = ["timeout", "120", "tail", "-n", "+1", "-F", root.logPath];
