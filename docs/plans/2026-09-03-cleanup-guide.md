...
**Пока это не починено — ничего не удалять.** Снёс лишнее, передумал, вернуть
не можешь.
...
## Незакрытый хвост: NoExtract лежит не в той секции

```bash
grep -n "NoExtract" /etc/pacman.conf
```

Сейчас строки стоят в конце файла, внутри секции репозитория `[g14]`, где
...
| `volantes_cursors` | тема курсора, на неё ссылаются `gtk-3.0/settings.ini` обоих ригов — **поставить**, а не убирать |
...
1. Починить зеркала и подписи. Без этого дальше не идти.
2. Поправить `NoExtract` в живом `/etc/pacman.conf`.
