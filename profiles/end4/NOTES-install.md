...
**✅ ВЕРИФИКАЦИЯ БИНДОВ (2026-07-16, nested fake-HOME, БЕЗ relogin):**
Минимальный nested-конфиг: симулировал ii-дефолты (SUPER+Tab/M/K) → `dofile`
custom/keybinds.lua → `hyprctl binds` дамп. Итог:
- `hl.unbind("SUPER + Tab")` **РАБОТАЕТ** — SUPER+Tab в дампе РОВНО 1× (modmask 64),
  ii-дефолт снят, **НЕ стекается**. Арг-форма (строка комбо) верна.
- Полный файл загрузился: 90 биндов (72 hl.bind + циклы), **ноль lua-ошибок**
  (traceback пуст). resize-инлайн не крашнул.
- 3 главных риска СНЯТЫ статик-верификацией. Осталось для Task 13 relogin: живое
  поведение (реальные окна/шелл-бар), но конфиг-логика доказана.
